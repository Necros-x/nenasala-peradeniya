...
export default function LessonView() {
  ...
  if (loading) return <div className="p-8"><Skeleton className="h-[60vh] w-full" /></div>;
  ...
  const LessonIcon = ({ type, completed }: { type: string, completed?: boolean }) => {
    if (completed) return <CheckCircle2 className="w-4 h-4 text-[var(--color-success)]" />;
    if (type === 'video') return <PlayCircle className="w-4 h-4 text-[var(--color-text-muted)]" />;
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] md:h-[calc(100vh-8rem)] -mx-4 -mt-4 md:-mx-8 md:-mt-8 bg-[var(--color-background)]">
      ...
      <div className="h-14 bg-[var(--color-surface)] border-b border-[var(--color-border)] flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-3">
          <Link to={`/courses/${courseId}`} className="text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors">
            ...
            <div className="max-w-5xl mx-auto w-full flex-1 flex flex-col p-4 md:p-8">
              ...
              <div className="flex-1 bg-[var(--color-surface)] rounded-[var(--radius-lg)] shadow-sm border border-[var(--color-border)] overflow-hidden flex flex-col">
                ...
                <div className="p-6 md:p-10 flex-1">
                  <h1 className="text-2xl md:text-3xl font-bold mb-6 text-[var(--color-text-primary)]">{currentLesson.title}</h1>
                  ...
                  {currentLesson.type === 'document' && (
                    <div className="p-8 border-2 border-dashed border-[var(--color-border)] rounded-[var(--radius-md)] text-center">
                      ...
                      <h3 className="font-semibold text-lg mb-2">Document Resource</h3>
              ...
              <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                >
                  {completing ? 'Completing...' : (
                    <>
                      Mark Complete {nextLesson && '& Continue'} <ChevronRight className="w-4 h-4 ml-2" />
                    </>
                  )}
                </Button>
              </div>
        ...
        `}>
          ...
          <div className="flex-1 overflow-y-auto">
            {course.modules.map((mod, mIdx) => (
              <div key={mod.id} className="border-b border-[var(--color-border)] last:border-0">
                  ...
                  {mod.lessons.map((lesson, lIdx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`text-sm font-medium leading-tight ${isActive ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-primary)]'}`}>
                          {lIdx + 1}. {lesson.title}
                          {lesson.duration && (
                            <div className="text-xs text-[var(--color-text-muted)] mt-1 font-normal">{lesson.duration} min</div>
                          )}
                        </div>
                      </Link>
                    )
